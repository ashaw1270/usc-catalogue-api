...
def parse_program_html_v2(html: str, catoid: int, poid: int, slug: str | None = None) -> ProgramV2:
    ...
    blocks: list[RequirementBlockV2] = []
    seen: set[str] = set()
...
async def fetch_program_v2(catoid: int, poid: int, slug: str | None = None) -> ProgramV2:
    """Fetch program page from USC catalogue and return parsed ProgramV2."""
    url = f"{settings.catalogue_base_url.rstrip('/')}/preview_program.php"
    params = {"catoid": catoid, "poid": poid}
    async with httpx.AsyncClient(timeout=settings.http_timeout_seconds) as client:
        response = await client.get(url, params=params)
        response.raise_for_status()
    return parse_program_html_v2(response.text, catoid, poid, slug)
